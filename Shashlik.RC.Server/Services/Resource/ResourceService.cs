...
{
    public ResourceService(RoleService roleService, IFreeSql dbContext, IHttpContextAccessor httpContextAccessor,
    ...
    {
        var list = await DbContext.Select<IdentityUserRole<int>, IdentityRole<int>, IdentityRoleClaim<int>>()
            .InnerJoin((userRole, role, roleClaim) => userRole.RoleId.Equals(role.Id))
            .InnerJoin((userRole, role, roleClaim) => role.Id.Equals(roleClaim.RoleId))
            .Where((userRole, role, roleClaim) => userRole.UserId.Equals(userId))
            .Distinct()
            .ToListAsync((userRole, role, roleClaim) => new
            {
