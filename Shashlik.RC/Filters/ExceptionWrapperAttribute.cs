        ...
        public override void OnException(ExceptionContext context)
        {
            if (context.ActionDescriptor is ControllerActionDescriptor actionDescriptor)
            {
                ...
                for (var i = 0; i < FindDepthResponseException; i++)
                {
                    ...
                    {
                        var errorCode = responseException.ErrorCode;
                        var message = responseException.Message;

                        var responseResult = new ResponseResult(errorCode, false, message, responseException.Data, null);
                        ...
                        logger.LogInformation(context.Exception,
                            $"Request has wrapped ResponseResult, debug: {responseResult.Debug ?? "empty"}");
                        return;
                    }
                ...
                // 没有ResponseException异常,包装500系统错误
                {
                    ...
                    var debug = exception.ToString();
                    var responseResult = new ResponseResult((int)ResponseStatus.SystemError, false,
                        "inner server error", null, debug);
